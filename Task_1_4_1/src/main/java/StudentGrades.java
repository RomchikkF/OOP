...
public class StudentGrades {
    ...
    public Grade qualificationTaskGrade = null;
    ...
    public StudentGrades(HashMap<Subject, ArrayList<Grade>> grades) {
        this.grades = grades;  // (HashMap<Subject, ArrayList<Grade>>) grades.clone();
    }

    public StudentGrades(HashMap<Subject, ArrayList<Grade>> grades, Grade qualificationTaskGrade) {
        this.grades = grades;  // (HashMap<Subject, ArrayList<Grade>>) grades.clone();
        this.qualificationTaskGrade = qualificationTaskGrade;
    }
    ...
    public boolean canGetRedDiploma() {
        boolean result =
                grades.keySet().stream().noneMatch(subj -> getLastGrade(subj) == Grade.Unsatisfactory) &&
                grades.keySet().stream().noneMatch(subj -> getLastGrade(subj) == Grade.Satisfactory) &&
                getAverageGrade() >= 4.75f &&
                qualificationTaskGrade == Grade.Great;
        return result;
    }
